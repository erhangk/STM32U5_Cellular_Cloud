...
#define THREAD_DECLARE(A)                         uint32_t (A)

#define THREAD_INIT(A, THREAD_FUNC, THREAD_CONTEXT, STACKSIZE, PRIORITY)  THREAD_OK
...
/**
  * @brief  Pop msg from FIFO queue without OS
  * @param  q: pointer to the queue
  * @param  timeout: timeout in milliseconds
  * @param  idle_func: the function call this idle_func internally before timeout
  * @retval pointer to the msg get from the queue, NULL if failed.
  */
