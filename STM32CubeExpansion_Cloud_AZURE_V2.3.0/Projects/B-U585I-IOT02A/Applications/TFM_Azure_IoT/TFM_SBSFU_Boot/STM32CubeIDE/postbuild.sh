...
echo Copying bootloader binary
#Make sure we have a Binary sub-folder in SBSFU folder
if [ ! -e $binarydir ]; then
echo "create Binary dir"
mkdir $binarydir
fi
cp $projectdir/Release/TFM_SBSFU_Boot.bin $binarydir/bootloader.bin
...
echo $command
$command >> $projectdir"/output.txt" 2>&1
ret=$?
if [ $ret != 0 ]; then
