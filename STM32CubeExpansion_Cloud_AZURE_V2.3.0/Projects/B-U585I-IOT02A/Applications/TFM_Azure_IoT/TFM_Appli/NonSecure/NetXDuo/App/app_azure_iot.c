...
#ifdef ENABLE_ADU
extern VOID app_adu_start(NX_AZURE_IOT_HUB_CLIENT *hub_client_ptr);
#endif /* ENABLE_ADU */
...
NX_AZURE_IOT_JSON_WRITER json_writer;
    ...
    while (1)
    {
        ...
        {
            printf("Successfully executed command %.*s on %.*s\r\n", command_name_length, command_name_ptr,
...
ULONG properties_version;
    ...
    if ((status = nx_azure_iot_hub_client_writable_properties_receive(hub_client_ptr,
                                                                      ...
                                                                      NX_WAIT_FOREVER)))
    {
        printf("Receive writable property failed!: error code = 0x%08x\r\n", status);
        return;
    }
...
ULONG writable_properties_version;
    ...
    if ((status = nx_azure_iot_hub_client_properties_receive(hub_client_ptr,
                                                             ...
                                                             NX_WAIT_FOREVER)))
    {
        printf("Receive all properties  failed!: error code = 0x%08x\r\n", status);
        return;
    }
