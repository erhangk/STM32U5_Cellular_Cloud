...
extern INT internal_version_compare(psa_image_id_t image_id, const UCHAR *buffer_ptr, UINT buffer_len);
...
psa_status_t status;
    ...
    switch (driver_req_ptr -> nx_azure_iot_adu_agent_driver_command)
    {
        ...
        case NX_AZURE_IOT_ADU_AGENT_DRIVER_UPDATE_CHECK:
        {
            ...
            /* Read the version of non secure image and compare the version in update_id to check if the update is installed or not.
               If installed, return NX_TRUE, else return NX_FALSE.  */
            if (internal_version_compare(image_id_ns_running, 
        ...
        case NX_AZURE_IOT_ADU_AGENT_DRIVER_WRITE:
        {
            ...
            */
            status = internal_flash_write(image_id_ns,
        ...
        case NX_AZURE_IOT_ADU_AGENT_DRIVER_INSTALL:
        {
            ...
            status = psa_fwu_install(image_id_ns, &dependency_uuid, &dependency_version);
            ...
            if (psa_fwu_query(image_id_ns, &info) != PSA_SUCCESS)
            {
                driver_req_ptr -> nx_azure_iot_adu_agent_driver_status = NX_AZURE_IOT_FAILURE;
                break;
            }
