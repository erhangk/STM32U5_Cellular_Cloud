...
set /p version=<"%projectdir%\..\NonSecure\VERSION" 
) ELSE IF "%signing%" == "secure" ( 
set /p version=<"%projectdir%\..\Secure\VERSION" 
)
)
...
set image_ns_size=0xa0000
set image_s_size=0x30000
set primary_only=0
...
set "tfm_s=%projectdir%\Secure\B-U585I-IOT02A\Exe\Project.bin"
set "tfm_ns=%projectdir%\NonSecure\B-U585I-IOT02A\Exe\Project.bin"
...
set "option=%option% --boot-record=SPE"
:signing
echo Processing binary: %signing% %version% %mode% %option% app_image_number=%app_image_number%
...
echo %signing% clear image signing
...
echo %signing% enc image encrypting and signing
